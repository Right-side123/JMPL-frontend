...
//         const response = await fetch(`${API_URL}/agents?manager_id=${managerId}&start_date=${formattedStartDate}&end_date=${formattedEndDate}`);
//         if (!response.ok) {
//           throw new Error('Failed to fetch agents');
//         }
...
//       (totals, agent) => ({
//         totalCalls: totals.totalCalls + (parseInt(agent.totalCalls) || 0),
//         totalConnected: totals.totalConnected + (parseInt(agent.totalConnected) || 0),
//         totalNotConnected: totals.totalNotConnected + (parseInt(agent.totalNotConnected) || 0),
//         totalOutbound: totals.totalOutbound + (parseInt(agent.totaloutbound) || 0),
//         totalIncomming: totals.totalIncomming + (parseInt(agent.totalincomming) || 0),
//         totalMissedOutbound: totals.totalMissedOutbound + (parseInt(agent.totalMissedOutbound) || 0),
//         totalAbandoned: totals.totalAbandoned + (parseInt(agent.totalAbandoned) || 0),
//       }),
//       {
//         totalCalls: 0,
//         totalConnected: 0,
...
//     const sheetData = [dateTimeRow, blankRow, headers, ...dataWithHeaders.map(row => Object.values(row)), Object.values(totalRow)];
//     const ws = XLSX.utils.aoa_to_sheet(sheetData);
//     const wb = XLSX.utils.book_new();
//     XLSX.utils.book_append_sheet(wb, ws, 'CDR Data');
//     XLSX.writeFile(wb, 'Agents.xlsx');
...
//         <div className="buttons_container">
//           <button onClick={handleBackAgents}>Back</button>
//           <button onClick={downloadExcel}>Download</button>
//         </div>
//       </div>

//       <div className="agent-container">
//         {loading ? <p>Loading agents...</p> : error ? <p>Error: {error}</p> : (
//           <table className="agents-table">
//             <thead>
//               <tr>
//                 <th>S.No.</th>
...
//             <tbody>
//               {filteredAgents.map((agent, index) => (
//                 <tr key={agent.agentmobile} onClick={() => handleAgentClick(agent.agentmobile)}>
//                   <td>{index + 1}</td>
//                   <td>{agent.agentname}</td>
//                   <td>{agent.agentmobile}</td>
//                   <td>{agent.status}</td>
...
//                   <td>{agent.totalConnected}</td>
//                   <td>{agent.totalNotConnected}</td>
//                   <td>{agent.totaloutbound}</td>
//                   <td>{agent.totalincomming}</td>
//                   <td>{agent.totalMissedOutbound}</td>
//                   <td>{agent.totalAbandoned}</td>
//                 </tr>
//               ))}
//             </tbody>
//           </table>
